...
const EVENTS0 = [
  { id:1, ch:"whatsapp", name:"Sarah Mitchell", av:"SM", elapsed:"2m",  intent:"BOOKING",   urgency:"HIGH",   status:"PENDING",   conf:96, preview:"Any slots Friday for a cut and colour?", msg:"Hi, do you have availability this Friday for a cut and colour?", reply:"Hi Sarah! We have Friday at 2pm or 4pm. Which suits you?", action:"CREATE_BOOKING", summary:"Booking Friday Cut and Colour", data:{ Service:"Cut and Colour", Day:"Friday" } },
  { id:2, ch:"email",    name:"James Okafor",   av:"JK", elapsed:"14m", intent:"INVOICE",   urgency:"MEDIUM", status:"PENDING",   conf:99, preview:"Please send an invoice for last Tuesday.", msg:"Hi, please could you send an invoice for last Tuesday's deep tissue massage?", reply:"Hi James! Of course, your invoice is on its way now.", action:"CREATE_INVOICE", summary:"Invoice Deep Tissue Insurance", data:{ Service:"Deep Tissue", Date:"Tue 8 Apr" } },
  { id:3, ch:"instagram",name:"@glowupldn",     av:"GL", elapsed:"32m", intent:"ENQUIRY",   urgency:"MEDIUM", status:"PENDING",   conf:91, preview:"Do you do lash lifts? How much?", msg:"Do you do lash lifts? How much?", reply:"Hey! Yes we do. Lash lifts are 45 and take about 45 mins. Want me to check availability?", action:"REPLY", summary:"Instagram DM lash lift pricing", data:{} },
  ...
  { id:7, ch:"email",    name:"Tom Hendricks",  av:"TH", elapsed:"3h",  intent:"GENERAL",   urgency:"LOW",    status:"AUTO",      conf:78, preview:"Confirming my session tomorrow.", msg:"Just confirming I am still on for tomorrow at 7:30am!", reply:"Hi Tom! Confirmed, see you tomorrow at 7:30am.", action:"REPLY", summary:"Confirmation 7:30am", data:{} },
];
...
const NAV = [
  ...
  { label:"Blog",     page:"website",  id:"top" },
];
const PRICING = [
  {
...
const FAQS = [
  { q:"What if Halo gets something wrong?", a:"Halo uses a confidence score on every message. At 95% confidence or above, it replies automatically. Between 60 and 94%, it sends you a one-tap approval on WhatsApp before anything goes out. Below 60%, it escalates to you directly. Nothing reaches a customer without meeting the threshold — and every decision is logged so you can see exactly why Halo acted the way it did." },
...
const COMPARISON = [
  ...
  { f:"Confidence-based routing", manual:"No",             bot:"No",                halo:"95% auto · 60–94% approval · <60% escalate" },
  ...
  { f:"Works while you sleep",    manual:"No",             bot:"Partially",         halo:"Always on, never off sick" },
];
...
const ConfidenceSection = () => {
  const T = T_(); const w = useW(); const mob = w < 768;
  const tiers = [
    { range:"95% +", label:"Auto-send", color:T.green, bg:T.greenBg, desc:"Halo replies instantly. No input needed from you. Common enquiries, confirmations, and pricing questions handled before you even see them." },
    { range:"60–94%", label:"Approval needed", color:T.yellow, bg:T.yellowBg, desc:"Halo sends you a one-tap WhatsApp notification with the suggested reply. Approve, edit, or dismiss in seconds." },
    { range:"Below 60%", label:"Escalate to you", color:T.red, bg:T.redBg, desc:"Complex or sensitive messages come straight to you with full context. Halo never guesses when the stakes are high." },
  ];
  return (
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"16px 20px" }}>
          ...
          <div style={{ background:"#0A1A0E", borderRadius:8, padding:"12px 14px", marginBottom:14, border:"1px solid rgba(37,209,102,0.12)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:8 }}>
              ...
              <span style={{ fontSize:10, fontWeight:600, color:"#25D166", letterSpacing:"0.04em" }}>WHATSAPP NOTIFICATION SENT</span>
...
const PreviewPipelineView = () => {
  ...
  const run = async () => {
    ...
    try {
      const r = await fetch("/api/chat", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ model:"claude-sonnet-4-5", max_tokens:500, system:CLF_SYS, messages:[{ role:"user", content:"Customer message: " + msg }] }) });
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              <div style={{ background:T.raised, border:`1px solid ${T.border}`, borderRadius:10, padding:"14px 16px" }}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:7, marginBottom:10 }}>
                  {[{l:"Intent",v:res.intent,c:iC?.text||T.accent},{l:"Confidence",v:res.confidence+"%",c:res.confidence>80?T.green:T.yellow},{l:"Urgency",v:res.urgency,c:UC[res.urgency]||T.sub},{l:"Action",v:(res.suggestedAction||"").replace(/_/g," "),c:T.accent}].map(f => (
              ...
              <div style={{ background:"#0A1A0E", borderRadius:10, padding:"12px 14px", border:"1px solid rgba(37,209,102,0.12)" }}>
                <div style={{ fontSize:10, fontWeight:600, color:"#3A8A4A", letterSpacing:"0.06em", marginBottom:7 }}>WHATSAPP NOTIFICATION</div>
                <div style={{ fontSize:12.5, color:"#A0D4B0", lineHeight:1.65 }}>
                  <strong style={{ color:"#DCF8E4" }}>New {res.intent}</strong> - {res.urgency}<br/>
  ...
  const send = async text => {
    ...
    try {
      const r = await fetch("/api/chat", {
        ...
        body:JSON.stringify({ model:"claude-sonnet-4-5", max_tokens:600, system:AI_SYS, messages:m })
      });
      const d = await r.json();
...
const NOTIF_EXAMPLES = [
  ...
  { name:"@glowupldn",     ch:"Instagram",msg:"Do you do lash lifts? How much?", intent:"ENQUIRY", conf:78, reply:"Hey! Yes, lash lifts are £45 and take about 45 mins. Want me to check availability for you?", av:"GL", handled:false },
  { name:"Luke Morrison",  ch:"WhatsApp", msg:"I want to cancel my appointment. Been waiting 3 days for a reply.", intent:"COMPLAINT", conf:54, reply:null, av:"LM", handled:false, escalated:true },
];
...
const PreviewNotificationsView = () => {
  ...
  // The one that needs approval (confidence 78%)
  const flagged = NOTIF_EXAMPLES[2];
...
const FeaturesTeaserStrip = ({ onNav }) => {
  const T = T_(); const w = useW(); const mob = w < 768;
  const cards = [
    { label:"Confidence-Based Automation", desc:"95% confident? Auto-sends. Below that? You get a one-tap approval. Nothing slips through.", page:"features", id:"confidence" },
    ...
    <section style={{ padding:`${mob?80:120}px ${mob?20:28}px`, borderTop:`1px solid ${T.border}`, background:T.page }}>
      <div style={{ maxWidth:640, margin:"0 auto", textAlign:"center", position:"relative" }}>
        ...
        <FadeIn><div style={{ position:"relative" }}>
          ...
          <p style={{ fontSize:mob?15:18, color:T.sub, lineHeight:1.7, marginBottom:36, maxWidth:500, margin:"0 auto 36px" }}>Halo isn't just selling AI. It's selling time, peace of mind, and a business that keeps running even when you're with a client, with your family, or fast asleep. Set up in 10 minutes. Free for 14 days.</p>
  ...
  return (
    <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:14, overflow:"hidden", height:"100%" }}>
      ...
      <div style={{ padding:"16px 20px", display:"flex", flexDirection:"column", gap:12 }}>
        ...
        <div>
          <div style={{ fontSize:11, fontWeight:600, color:T.muted, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:7 }}>Suggested Reply</div>
  ...
  <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100%", padding:"20px" }}>
    <div style={{ background:T.isDark?"#0B1410":"#F0FAF1", borderRadius:20, padding:"22px", border:`1px solid ${T.isDark?"rgba(37,209,102,0.14)":"rgba(37,209,102,0.3)"}`, maxWidth:340, width:"100%" }}>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16, paddingBottom:14, borderBottom:`1px solid ${T.isDark?"rgba(255,255,255,0.06)":"rgba(0,0,0,0.06)"}` }}>
        ...
        <div><div style={{ fontSize:15, fontWeight:700, color:T.isDark?"#D9FDD3":"#111" }}>Halo</div><div style={{ fontSize:12, color:T.isDark?"#4A8A5A":"#555" }}>Business AI online</div></div>
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:5 }}><div style={{ width:7, height:7, borderRadius:"50%", background:"#25D166", animation:"halo-pulse 2s ease-in-out infinite" }}/><span style={{ fontSize:11, color:"#25D166", fontWeight:600 }}>Active</span></div>
      </div>
      <div style={{ background:T.isDark?"#1A3020":"#D9FDD3", borderRadius:"4px 16px 16px 16px", padding:"13px 15px", marginBottom:12 }}>
        <div style={{ fontSize:11.5, fontWeight:700, color:"#25D166", letterSpacing:"0.04em", marginBottom:5 }}>NEW BOOKING - URGENT</div>
        <div style={{ fontSize:13.5, color:T.isDark?"#C8EED4":"#111", lineHeight:1.8 }}>
          <span style={{ color:T.isDark?"#6AAA7A":"#444" }}>From: </span><span style={{ fontWeight:600 }}>Sarah Mitchell</span><br/>
          <span style={{ color:T.isDark?"#6AAA7A":"#444" }}>Service: </span>Cut and Colour - Friday
        </div>
      </div>
...
const FtPipelineVisual = ({ T }) => (
  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, alignContent:"start" }}>
    <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:12, padding:"18px" }}>
      ...
      {["Message received","Intent classified","Reply generated","Notification sent"].map((s,i) => (
        <div key={i} style={{ display:"flex", gap:10, alignItems:"center", marginBottom:i<3?12:0 }}>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      ...
      <div style={{ background:"#0A1A0E", borderRadius:12, padding:"14px", border:"1px solid rgba(37,209,102,0.12)" }}>
        <div style={{ fontSize:10, fontWeight:600, color:"#3A8A4A", letterSpacing:"0.07em", marginBottom:7 }}>WHATSAPP NOTIFICATION</div>
        <div style={{ fontSize:12, color:"#A0D4B0", lineHeight:1.7 }}><strong style={{ color:"#DCF8E4" }}>New BOOKING - Urgent</strong><br/><em style={{ color:"#88CCA0" }}>2pm or 4pm Friday?</em></div>
      </div>
    </div>
  </div>
);
  ...
  return (
    <div data-scroll="" style={{ background:T.page, color:T.text, height:"100vh", overflowY:"auto", transition:"background 0.25s,color 0.25s" }}>
      ...
      <section style={{ padding:`${mob?64:96}px ${mob?20:28}px`, borderTop:`1px solid ${T.border}`, background:T.page }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"repeat(3,1fr)", gap:20 }}>
            {[{Icon:Bot,t:"AI-generated, always accurate",d:"Halo reads the request, matches it to your service list, applies VAT if applicable, and builds the invoice. No templates, no manual entry."},{Icon:Check,t:"You approve before anything sends",d:"Every invoice lands in your dashboard first. Review, edit, or reject it entirely. Nothing leaves without your approval."},{Icon:MessageSquare,t:"Payment tracking built in",d:"Once sent, Halo marks the invoice as outstanding. When the payment clears, it logs it as paid and notifies you."}].map((c,i) => (
              <FadeIn key={i} delay={i*80}><div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:12, padding:"24px 26px" }}>
...
const ApprovalFlow = ({ ev, onUpdate }) => {
  ...
  useEffect(() => {
    ...
    if (step===2 && isInv && !invoice && !fetching) {
      ...
      fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5",max_tokens:500,system:INVOICE_PROMPT,messages:[{role:"user",content:"Invoice for "+svc+" on "+date+" for "+ev.name+". Small UK service business."}]})})
      ...
      {(!isMobile||showD) && cur && (
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:`18px 22px`,paddingBottom:isMobile?80:18}}>
            ...
            {cur.reply && <><div style={{fontSize:11,fontWeight:600,color:T.muted,textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:8}}>Suggested Reply</div><div style={{fontSize:13.5,color:T.text,background:T.raised,padding:"13px 15px",borderRadius:8,lineHeight:1.75,marginBottom:18,border:`1px solid ${T.border}`,borderLeft:`3px solid ${T.green}`}}>{cur.reply}</div></>}
...
const DbAssistantView = ({ isMobile }) => {
  ...
  const send=async text=>{
    ...
    try{const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5",max_tokens:1000,system:"You are Halo, an AI business assistant for small UK service businesses. Be concise, warm, and practical.",messages:m})});const d=await r.json();setMsgs(p=>[...p,{role:"assistant",content:d.content?.find(b=>b.type==="text")?.text||"Something went wrong."}]);}
