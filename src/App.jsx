...
<style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Serif+Display&family=Inter:wght@300;400;500;600;700&display=swap'); *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; } html { scroll-behavior:smooth; } body { font-family:'Inter',system-ui,sans-serif; -webkit-font-smoothing:antialiased; } *, *::before, *::after { box-sizing: border-box; } html, body { overflow-x: hidden; max-width: 100vw; } img, video, iframe, svg { max-width: 100%; } ::selection { background:${T.accentBg}; } ::-webkit-scrollbar { width:5px; } ::-webkit-scrollbar-track { background:transparent; } ::-webkit-scrollbar-thumb { background:${T.border}; border-radius:99px; } input::placeholder, textarea::placeholder { color:${T.muted}; } @keyframes halo-bob    { 0%,100%{transform:translateY(0)} 50%{transform:translateY(5px)} } @keyframes halo-bounce { 0%,100%{transform:translateY(0);opacity:0.4} 50%{transform:translateY(-3px);opacity:1} } @keyframes halo-spin   { from{transform:rotate(0deg)} to{transform:rotate(360deg)} } @keyframes halo-cursor { 0%,100%{opacity:1} 50%{opacity:0} } @keyframes halo-msg-in { from{opacity:0;transform:translateY(6px)} to{opacity:1;transform:translateY(0)} } @keyframes halo-screen { from{opacity:0} to{opacity:1} } @keyframes halo-pulse  { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:0.5;transform:scale(0.85)} }`}</style>
